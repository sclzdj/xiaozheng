...
use app\alluse\model\City as CityModel;
use think\Db;
use think\Request;
use think\Validate;
/**
...
class City extends Admin
{
    ...
    public function add($action='',$pid='')
    {
        ...
        if($action=='ch'){
            ...
            return ZBuilder::make('form')
                ->setPageTitle('添加'.get_comment('city')) // 设置页面标题
                ...
                ->addText('title', '名称', '必填','')
                ->addFormItems([
                    ['select', 'pid', '所属'.get_comment('city'), '所属上级'.get_comment('city'), CityModel::getCityTree($pid,'',false), $pid],
                ])
    ...
    public function edit($id='')
    {
        ...
        if ($id>0) {
            ...
            // 使用ZBuilder快速创建表单
            return ZBuilder::make('form')
                ->setPageTitle('修改'.get_comment('city')) // 设置页面标题
        ...
        if ($this->request->isPost()) {
            ...
            if (!empty($data)) {
                ...
                foreach ($tables as $key => $table) {
                    ...
                    foreach ($shis as $k => $v) {
                        Db::name($table)->where('shi',$v)->update(['sheng'=>Db::name('city')->where('id',$v)->value('pid')]);
                    }
                }
    ...
    private function getNestCity($lists = [], $max_level = 0, $pid = 0, $curr_level = 1)
    {
        ...
        foreach ($lists as $key => $value) {
            if ($value['pid'] == $pid) {
                ...
                if($value['id']!=1){
                    $result .= '<a href="'.url('add', ['action'=>'ch','pid' => $value['id']]).'" data-toggle="tooltip" data-original-title="新增子'.get_comment('city').'"><i class="list-icon fa fa-plus fa-fw"></i></a><a href="'.url('edit', ['id' => $value['id']]).'" data-toggle="tooltip" data-original-title="编辑"><i class="list-icon fa fa-pencil fa-fw"></i></a>';
                }else{
                    $result .= '<a href="'.url('add', ['action'=>'ch','pid' => $value['id']]).'" data-toggle="tooltip" data-original-title="新增子'.get_comment('city').'"><i class="list-icon fa fa-plus fa-fw"></i></a>';
                }
