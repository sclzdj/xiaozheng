...
class Basis extends Admin
{
	public function index($is_del='0'){ 
	    ...
	    switch ($is_del) {
	    	case '0':
		        ...
		        $map = $this->getMap();
				$data_list = Db::name('basis')->where('is_del','0')->where($map)->order($order)->paginate();
				$page = $data_list->render();
	            return ZBuilder::make('table')
		        	->setPageTitle('基础信息字段列表') // 设置页面标题
		        	->setPageTips('某些操作执行之后可能会导致其他的相关数据失效，所以请谨慎操作') // 设置页面提示信息
		        	->setTabNav($list_tab,  $is_del)//分组
		        	->setTableName('basis') // 指定数据表名
	        ...
	        case '1':
		        ...
		        $map = $this->getMap();
				$data_list = Db::name('basis')->where('is_del','1')->where($map)->order($order)->paginate();
				$page = $data_list->render();
	        	return ZBuilder::make('table')
		        	->setPageTitle('基础信息字段列表') // 设置页面标题
		        	->setPageTips('某些操作执行之后可能会导致其他的相关数据失效，所以请谨慎操作') // 设置页面提示信息
	...
	public function sort(){
		...
		if (Request::instance()->isPost()) {
			...
			foreach ($data['sorts'] as $k => $v) {
				Db::name('basis')->update(['id'=>$v,'sort'=>$k+1]);
			}
			//跳转
			return $this->success('调整排序成功','index','',1);
		}
		$basiss=Db::name('basis')->where('is_del','0')->order('sort asc,id desc')->select();
		$list=[];
		foreach ($basiss as $k => $v) {
			$title=$v['title'];
	...
	public function del(){
		...
		$rt=Db::name('basis')->where('id','in',$ids)->update(['is_del'=>'1']);
		if($rt!==false){
	...
	public function restore(){
		$ids = (Request::instance()->isGet()) ? input('ids') : input('post.ids/a');
		$rt=Db::name('basis')->where('id','in',$ids)->update(['is_del'=>'0']);
		if($rt!==false){
	...
	public function add(){
		...
		if (Request::instance()->isPost()) {
			...
			//数据入库
			$basis_id=Db::name("basis")->insertGetId($insert);
			//跳转
			if($basis_id>0){
	...
	public function edit($id=''){
		...
		if (Request::instance()->isPost()) {
			...
			//数据更新
			$rt=Db::name("basis")->update($update);
			//跳转
			if($rt!==false){
		...
		if ($id>0) {
			// 查处数据
			$basis=Db::name("basis")->where('id',$id)->find();
			if(!$basis){
				return $this->error('请求错误');
			}
			// 使用ZBuilder快速创建表单
			return ZBuilder::make('form')
				->setPageTitle('修改基础信息字段') // 设置页面标题
				->setPageTips('请认真修改相关信息') // 设置页面提示信息
				//->setUrl('edit') // 设置表单提交地址
				//->hideBtn(['back']) //隐藏默认按钮
				->setBtnTitle('submit', '确定') //修改默认按钮标题
				->addBtn('<button type="reset" class="btn btn-default">重置</button>') //添加额外按钮
				->addText('title', '名称','',$basis['title'])
				->addSelect('type', '字段类型','',['text'=>'单行文本','textarea'=>'多行文本','select'=>'下拉选择'],$basis['type'])
				->addTextarea('options', '下拉类型选项','一行为一个选项',$basis['options'])
				->addText('sort', '排序','此项须为整数',$basis['sort'])
				->addHidden('id',$basis['id'])
				->setTrigger('type', 'select', 'options')
				//->isAjax(false) //默认为ajax的post提交
				->fetch();
		}
