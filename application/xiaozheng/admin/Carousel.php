...
class Carousel extends Admin
{
	    ...
	    switch ($is_del) {
	    	case '0':
	            ...
	            return ZBuilder::make('table')
		            ...
		            ->addFilter('module',['0'=>'首页','1'=>'内页']) // 添加字段筛选
		            ->addFilter('type',['0'=>'动态','1'=>'政策']) // 添加字段筛选
		            ->addFilterMap('type,module', ['is_del'=>0])//筛选条件
		        	->addColumns([
		        			['id', 'ID'],
		        			['module','页面模块','callback','array_v',['0'=>'首页','1'=>'内页']],
		        			['title', '标题'],
		        			['pic_id', '图片','picture','暂无图片'],
		        			['type','关联模块','callback','array_v',['0'=>'动态','1'=>'政策']],
		        			['relate_id','关联内容','callback',function($relate_id,$data){
		        				if($data['type']=='政策'){
		        					$policy=Db::name('policy')->find($relate_id);
									$title=$policy['title'];
									if($policy['is_del']) $title.='(已移除)';
									$class=Db::name('class')->find($policy['class_id']);
									if($class['is_del']){
										$class_title=$class['title'].'(已移除)';
									}else{
										$class_title=$class['title'];
									}
									$city=Db::name('city')->where('id',$policy['city_id'])->value('title');
									return "<a href='".url('xiaozheng/policy/look',['id'=>$relate_id])."' target='_bank'>".issetArrOffset($title)."</a><br><span style='font-size:10px;color:#999;'>{$class_title}</span>&nbsp;&nbsp;<span style='font-size:10px;color:#999;'>{$city}</span>";
		        				}else{
		        					$news=Db::name('news')->find($relate_id);
									$title=$news['title'];
									if($news['is_del']) $title.='(已移除)';
									$class=Db::name('class')->find($news['class_id']);
									if($class['is_del']){
										$class_title=$class['title'].'(已移除)';
									}else{
										$class_title=$class['title'];
									}
									$city=Db::name('city')->where('id',$news['city_id'])->value('title');
									return "<a href='".url('xiaozheng/news/look',['id'=>$relate_id])."' target='_bank'>".issetArrOffset($title)."</a><br><span style='font-size:10px;color:#999;'>{$class_title}</span>&nbsp;&nbsp;<span style='font-size:10px;color:#999;'>{$city}</span>";
		        				}
		        			},'__data__'],
		    		...
		    		->addTopButton('custom',['title'=>'前端排序(首页)','href'=>url('sort',['module'=>'0']),'icon'=>'glyphicon glyphicon-sort'])
		    		->addTopButton('custom',['title'=>'前端排序(内页)','href'=>url('sort',['module'=>'1']),'icon'=>'glyphicon glyphicon-sort']) 
	...
	public function sort($module='0'){
		...
		$carousels=Db::name('carousel')->where('is_del','0')->where('module',$module)->order('sort asc,id desc')->select();
		$list=[];
		...
		return ZBuilder::make('form')
			->setPageTitle('轮播图'.$tips.'排序') // 设置页面标题
	...
	public function add(){
		...
		$news=Db::name('news')->order('is_del asc,addtime desc,id desc')->select();
		$select_news=[];
		foreach ($news as $k => $v) {
			$title=$v['title'];
			if($v['is_del']) $title.='<span>(已移除)</span>';
			$news_one=Db::name('news')->find($v['id']);
			$class=Db::name('class')->find($news_one['class_id']);
			if($class['is_del']){
				$class_title=$class['title'].'(已移除)';
			}else{
				$class_title=$class['title'];
			}
			$city=Db::name('city')->where('id',$news_one['city_id'])->value('title');
			$select_news[$v['id']]="[{$class_title}]&nbsp;[{$city}]&nbsp;".$title;
		}
		...
		foreach ($policy as $k => $v) {
			$title=$v['title'];
			if($v['is_del']) $title.='(已移除)';
			$policy_one=Db::name('policy')->find($v['id']);
			$class=Db::name('class')->find($policy_one['class_id']);
			if($class['is_del']){
				$class_title=$class['title'].'(已移除)';
			}else{
				$class_title=$class['title'];
			}
			$city=Db::name('city')->where('id',$policy_one['city_id'])->value('title');
			$select_policy[$v['id']]="[{$class_title}]&nbsp;[{$city}]&nbsp;".$title;
		}
		// 使用ZBuilder快速创建表单
		return ZBuilder::make('form')
			...
			->setBtnTitle('submit', '确定') //修改默认按钮标题
			->addBtn('<button type="reset" class="btn btn-default">重置</button>') //添加额外按钮
			->addSelect('module', '页面模块','',['0'=>'首页','1'=>'内页'])
			->addText('title', '标题','','')
			->addImage('pic_id', '图片','','')
	...
	public function edit($id=''){
		...
		if ($id>0) {
			...
			return ZBuilder::make('form')
				...
				->setBtnTitle('submit', '确定') //修改默认按钮标题
				->addBtn('<button type="reset" class="btn btn-default">重置</button>') //添加额外按钮
				->addSelect('module', '页面模块','',['0'=>'首页','1'=>'内页'],$carousel['module'])
