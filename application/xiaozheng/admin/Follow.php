...
class Follow extends Admin
{
	public function index($status='0'){  
		...
		$map = $this->getMap();
		$count_1=Db::name('Follow')->where($map)->value('count(id)');	
		$list_tab = [
	        '0' => ['title' => '关注列表('.$count_1.')', 'url' => url('index', ['status' => '0'])],
	    ];
	    ...
	    switch ($status) {
	    	case '0':
				...
				$data_list = Db::name('follow')->where($map)->order($order)->paginate();
				$page = $data_list->render();
	            return ZBuilder::make('table')
		        	->setPageTitle('关注列表') // 设置页面标题
		        	->setPageTips('某些操作执行之后可能会导致其他的相关数据失效，所以请谨慎操作') // 设置页面提示信息
		        	->setTabNav($list_tab,  $status)//分组
		        	->setTableName('follow') // 指定数据表名
		        	...
		        	->addColumns([
		        			...
		        			['addtime', '关注时间','datetime',no_font('未知')],
		        			['right_button', '操作', 'btn'],
		        		]) //添加多列数据
	...
	public function delete($record = [])
    {
        ...
        $ids=(array)$ids;
        foreach ($ids as $k => $v) {
        	$follow=Db::name($table)->find($v);
        	if(!$follow){
        		continue;
        	}
			Db::name($table)->delete($follow['id']);
			Db::name($follow['relate'])->where('id',$follow['relate_id'])->setDec('follow');
        }
