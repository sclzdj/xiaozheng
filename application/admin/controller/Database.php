        ...
        switch ($group) {
            case 'export':
                ...
                return ZBuilder::make('table')
                    ...
                    ->setPageTips('备份时请全选，备份好后请到还原数据库查看并下载保存到本地') // 设置页面提示信息
                    ->setPrimaryKey('name')
            ...
            case 'import':
                ...
                foreach ($glob as $name => $file) {
                    if(preg_match('/^\d{8,8}-\d{6,6}-\d+\.sql(?:\.gz)?$/', $name)){
                        ...
                        $info['name']     = $info['time'];
                        $info['url']      =config('host_url').ltrim(config('data_backup_path'),'.').$file->getFilename();

                        $data_list["{$date} {$time}"] = $info;
                    }
                }
                $data_list = !empty($data_list) ? array_values($data_list) : $data_list;
                $data_list=list_sort_by($data_list,'name','desc');
                // 自定义按钮
                $btn_import = [
                    'title' => '还原',
                ...
                return ZBuilder::make('table')
                    ->setPageTitle('数据库管理') // 设置页面标题
                    ->setPageTips('定期把备份好的数据下载到本地，还原请联系开发人员') // 设置页面提示信息
                    ->setPrimaryKey('time')
                    ->hideCheckbox()
                    ->setTabNav($tab_list, $group) // 设置tab分页
                    ->addColumns([ // 批量添加数据列
                        ['name', '备份名称', 'callback',function($name,$data){
                            return "<a href='{$data['url']}' title='点击下载'>".date('Ymd-His',$name)."</a>";
                        },'__data__'],
                        ['part', '卷数'],
                        ['compress', '压缩'],
                        ['size', '数据大小', 'byte'],
