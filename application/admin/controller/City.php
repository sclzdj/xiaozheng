...
class City extends Admin
{
    ...
    public function add($action='',$pid='')
    {
        ...
        if (Request::instance()->isPost()) {
            ...
            //跳转
            if($version_id>0){
                return $this->success('添加城市成功','index','',1);
            } else {
                return $this->error('添加城市失败');
            }
        }
        if($action=='ch'){
            // 使用ZBuilder快速创建表单
            return ZBuilder::make('form')
                ->setPageTitle('添加城市') // 设置页面标题
                ...
                ->addFormItems([
                    ['select', 'pid', '所属城市', '所属上级城市', CityModel::getCityTree($pid,'',false), $pid],
                ])
                ->addText('title', '名称', '必填','')
            ...
            return ZBuilder::make('form')
                ...
                ->setBtnTitle('submit', '确定') //修改默认按钮标题
                ->addBtn('<button type="reset" class="btn btn-default">重置</button>') //添加额外按钮
                ->addLinkage('sheng', '所属省份', '必须选择', $select_sheng, '', url('get_shi'), 'shi,qu')
    ...
    public function edit($id='')
    {
        ...
        if (Request::instance()->isPost()) {
            ...
            //跳转
            if($rt!==false){
                return $this->success('修改城市成功','index','',1);
            } else {
                return $this->error('修改城市失败');
            }
        }
        // 接收id
        if ($id>0) {
    ...
    /**
     * 递归解析城市
     * @param array $citys 城市数据
     * @param int $pid 上级城市id
     * @author 杜军
    ...
    private function getNestCity($lists = [], $max_level = 0, $pid = 0, $curr_level = 1)
    {
        ...
        foreach ($lists as $key => $value) {
            if ($value['pid'] == $pid) {
                ...
                $result .= '<a href="'.url('add', ['action'=>'ch','pid' => $value['id']]).'" data-toggle="tooltip" data-original-title="新增子城市"><i class="list-icon fa fa-plus fa-fw"></i></a><a href="'.url('edit', ['id' => $value['id']]).'" data-toggle="tooltip" data-original-title="编辑"><i class="list-icon fa fa-pencil fa-fw"></i></a>';
                if ($value['status'] == 0) {
                    // 启用
                ...
                $result .= '<a href="'.url('delete', ['id' => $value['id'], 'table' => 'city']).'" data-toggle="tooltip" data-original-title="删除" class="ajax-get confirm"><i class="list-icon fa fa-times fa-fw"></i></a></div>';
                $result .= '</div>';
