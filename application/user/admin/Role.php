...
class Role extends Admin
{
    ...
    {   //myfuc(2);
        ...
        $map = $this->getMap();
        // 数据列表
        $data_list = RoleModel::where($map)->order('create_time desc,id desc')->paginate();
        // 分页数据
        ...
        return ZBuilder::make('table')
            ...
            ->replaceRightButton(['id' => 2], '<button class="btn btn-danger btn-xs" type="button" disabled>不可操作</button>') // 修改id为1的按钮
    ...
    public function add()
    {
        ...
        if (!$menus) {
            ...
            $menus = MenuModel::where('module', 'in', $modules)->where($where)->order('sort,id')->column('id,pid,sort,title,icon');
